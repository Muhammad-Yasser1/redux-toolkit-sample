...
import { cartActions, cartInitialStateType } from './cart-slice';

export const updateCartOnServer =
	(cart: cartInitialStateType) => (dispatch: AppDispatch) => {
		dispatch(
...
export const initCart =
	(isCartInitiated: { current: boolean }) => (dispatch: AppDispatch) => {
		dispatch(
			uiActions.showNotification({
				title: 'Pending...',
			...
			.catch((err) => {
				...
				dispatch(
					uiActions.showNotification({
						...
						title: 'Error!',
						message:
							'Something went wrong when initiating your cart!',
					})
				);
			});
	};
